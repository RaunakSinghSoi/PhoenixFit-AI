...
const STORAGE_LAST_USER_ID = '@pfa_nutrition_last_user_v1';
...
export default function NutritionScreen() {
  ...
  async function bootstrap() {
    try {
      ...
      // Check if user changed - if so, clear local cache (but keep API key)
      const lastUserId = await AsyncStorage.getItem(STORAGE_LAST_USER_ID);
      if (lastUserId && lastUserId !== currentUserId) {
        // Different user - clear local cache
        console.log('[Nutrition] User changed, clearing local cache');
        await AsyncStorage.multiRemove([
          STORAGE_NUTRITION_BY_DATE,
          STORAGE_NUTRITION_GOALS,
        ]);
      }
      // Store current user ID
      await AsyncStorage.setItem(STORAGE_LAST_USER_ID, currentUserId);

      // Load API key (shared across users)
      const savedApiKeyRaw = await AsyncStorage.getItem(STORAGE_API_KEY);
      const savedKey = (savedApiKeyRaw || '').trim();
      if (savedKey) setRuntimeApiKey(savedKey);

      // Load from local storage
      const [rawByDate, rawGoals] = await Promise.all([
        AsyncStorage.getItem(STORAGE_NUTRITION_BY_DATE),
        AsyncStorage.getItem(STORAGE_NUTRITION_GOALS),
      ]);
      let parsedByDate: Record<string, DayNutrition> = rawByDate ? JSON.parse(rawByDate) : {};
      let parsedGoals: Goals = rawGoals ? JSON.parse(rawGoals) : DEFAULT_GOALS;
      ...
      if (user) {
        try {
          const nutritionDocRef = doc(firestore, 'users', user.uid, 'userData', 'nutrition');
          ...
          if (nutritionSnap.exists()) {
            ...
            parsedByDate = data.nutritionByDate || {};
            // Ensure structure on Firebase data
            for (const date of Object.keys(parsedByDate)) {
              parsedByDate[date] = ensureDayNutritionShape(parsedByDate[date]);
            }
            if (data.goals) {
              parsedGoals = { ...DEFAULT_GOALS, ...data.goals };
            }
            // Update local cache with Firebase data
            try { await AsyncStorage.setItem(STORAGE_NUTRITION_BY_DATE, JSON.stringify(parsedByDate)); } catch {}
            try { await AsyncStorage.setItem(STORAGE_NUTRITION_GOALS, JSON.stringify(parsedGoals)); } catch {}
          } else {
            // No Firebase data yet - start fresh for this user
            parsedByDate = {};
            parsedGoals = DEFAULT_GOALS;
            try { await AsyncStorage.setItem(STORAGE_NUTRITION_BY_DATE, JSON.stringify(parsedByDate)); } catch {}
            try { await AsyncStorage.setItem(STORAGE_NUTRITION_GOALS, JSON.stringify(parsedGoals)); } catch {}
          }
        } catch (e) {
          console.log('[Nutrition] Firebase fetch failed, using local data:', e);
        }
      } else {
        // Not logged in - start with empty/default data
        parsedByDate = {};
        parsedGoals = DEFAULT_GOALS;
      }
  ...
  async function persistByDate(next: Record<string, DayNutrition>) {
    ...
    if (user) {
      try {
        const nutritionDocRef = doc(firestore, 'users', user.uid, 'userData', 'nutrition');
        await setDoc(nutritionDocRef, { nutritionByDate: next }, { merge: true });
      } catch (e) {
        console.log('[Nutrition] Firebase save failed:', e);
      }
    }
  }
  ...
  async function persistGoals(next: Goals) {
    ...
    if (user) {
      try {
        const nutritionDocRef = doc(firestore, 'users', user.uid, 'userData', 'nutrition');
        await setDoc(nutritionDocRef, { goals: next }, { merge: true });
      } catch (e) {
        console.log('[Nutrition] Firebase save failed:', e);
      }
    }
          ...
          <View style={styles.modalRoot} pointerEvents="box-none">
            ...
            >
              ...
              <ScrollView keyboardShouldPersistTaps="handled">
                ...
                <TextInput value={mealForm.name} onChangeText={t => onMealFormChange('name', t)} placeholder="Chicken Bowl" placeholderTextColor="#9CA3AF" style={styles.input} />

                <View style={styles.row}>
                  <View style={styles.rowItem}>
                    <Text style={styles.inputLabel}>Calories</Text>
                    <TextInput value={mealForm.calories} onChangeText={t => onMealFormChange('calories', onlyNum(t))} placeholder="600" placeholderTextColor="#9CA3AF" style={styles.input} keyboardType="number-pad" />
                  </View>
                  <View style={styles.rowSpacer} />
                  <View style={styles.rowItem}>
                    <Text style={styles.inputLabel}>Protein (g)</Text>
                    <TextInput value={mealForm.protein} onChangeText={t => onMealFormChange('protein', onlyNum(t))} placeholder="40" placeholderTextColor="#9CA3AF" style={styles.input} keyboardType="number-pad" />
                  </View>
                </View>
                ...
                <View style={styles.row}>
                  <View style={styles.rowItem}>
                    ...
                    <TextInput value={mealForm.carbs} onChangeText={t => onMealFormChange('carbs', onlyNum(t))} placeholder="70" placeholderTextColor="#9CA3AF" style={styles.input} keyboardType="number-pad" />
                  </View>
                  <View style={styles.rowSpacer} />
                  <View style={styles.rowItem}>
                    <Text style={styles.inputLabel}>Fat (g)</Text>
                    <TextInput value={mealForm.fat} onChangeText={t => onMealFormChange('fat', onlyNum(t))} placeholder="20" placeholderTextColor="#9CA3AF" style={styles.input} keyboardType="number-pad" />
                  </View>
                </View>

                <Text style={styles.inputLabel}>Notes (optional)</Text>
                <TextInput value={mealForm.notes} onChangeText={t => onMealFormChange('notes', t)} placeholder="No dressing" placeholderTextColor="#9CA3AF" style={[styles.input, styles.notesInput]} multiline />
              </ScrollView>
        ...
        <Modal visible={waterModalVisible} animationType="slide" transparent onRequestClose={closeWaterModal}>
          <View style={styles.modalRoot} pointerEvents="box-none">
            ...
            >
              ...
              <TextInput value={waterInputOz} onChangeText={t => setWaterInputOz(onlyNum(t))} placeholder="12" placeholderTextColor="#9CA3AF" style={styles.input} keyboardType="number-pad" />
              <View style={styles.modalActions}>
              ...
              <ScrollView keyboardShouldPersistTaps="handled">
                <View style={styles.row}>
                  <View style={styles.rowItem}>
                    ...
                    <TextInput value={goalsForm.calories} onChangeText={t => setGoalsForm(prev => ({ ...prev, calories: onlyNum(t) }))} placeholder="2200" placeholderTextColor="#9CA3AF" style={styles.input} keyboardType="number-pad" />
                  </View>
                </View>
                <View style={styles.row}>
                  <View style={styles.rowItem}>
                    <Text style={styles.inputLabel}>Protein (g)</Text>
                    <TextInput value={goalsForm.protein} onChangeText={t => setGoalsForm(prev => ({ ...prev, protein: onlyNum(t) }))} placeholder="160" placeholderTextColor="#9CA3AF" style={styles.input} keyboardType="number-pad" />
                  </View>
                  <View style={styles.rowSpacer} />
                  <View style={styles.rowItem}>
                    <Text style={styles.inputLabel}>Carbs (g)</Text>
                    <TextInput value={goalsForm.carbs} onChangeText={t => setGoalsForm(prev => ({ ...prev, carbs: onlyNum(t) }))} placeholder="220" placeholderTextColor="#9CA3AF" style={styles.input} keyboardType="number-pad" />
                  </View>
                </View>
                <View style={styles.row}>
                  <View style={styles.rowItem}>
                    <Text style={styles.inputLabel}>Fat (g)</Text>
                    <TextInput value={goalsForm.fat} onChangeText={t => setGoalsForm(prev => ({ ...prev, fat: onlyNum(t) }))} placeholder="70" placeholderTextColor="#9CA3AF" style={styles.input} keyboardType="number-pad" />
                  </View>
                  <View style={styles.rowSpacer} />
                  <View style={styles.rowItem}>
                    <Text style={styles.inputLabel}>Water (oz)</Text>
                    <TextInput value={goalsForm.waterOz} onChangeText={t => setGoalsForm(prev => ({ ...prev, waterOz: onlyNum(t) }))} placeholder="64" placeholderTextColor="#9CA3AF" style={styles.input} keyboardType="number-pad" />
                  </View>
                </View>
              </ScrollView>
              <View style={styles.modalActions}>
