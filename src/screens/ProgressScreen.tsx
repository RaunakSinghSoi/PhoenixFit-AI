...
const STORAGE_LAST_USER_ID = '@pfa_progress_last_user_v1';
...
const RANK_TIERS: RankTier[] = [
  { name: 'Bronze', minDays: 5, color: '#CD7F32', icon: 'medal' },
  { name: 'Silver', minDays: 15, color: '#9CA3AF', icon: 'medal' },
  { name: 'Gold', minDays: 30, color: '#F59E0B', icon: 'trophy' },
  { name: 'Platinum', minDays: 50, color: '#06B6D4', icon: 'ribbon' },
  { name: 'Diamond', minDays: 75, color: '#6366F1', icon: 'sparkles' },
];
...
export default function ProgressScreen() {
  ...
  const [rankJustReached, setRankJustReached] = React.useState<RankTier | null>(null);
  ...
  async function bootstrap() {
    try {
      ...
      // Check if user changed - if so, clear local cache
      const lastUserId = await AsyncStorage.getItem(STORAGE_LAST_USER_ID);
      if (lastUserId && lastUserId !== currentUserId) {
        // Different user - clear local cache
        console.log('[Progress] User changed, clearing local cache');
        await AsyncStorage.multiRemove([
          STORAGE_EXERCISES_BY_DATE,
          STORAGE_DAYTYPE_BY_DATE,
        ]);
      }
      // Store current user ID
      await AsyncStorage.setItem(STORAGE_LAST_USER_ID, currentUserId);
      ...
      let parsedExercises: Record<string, Exercise[]> = rawExercises ? JSON.parse(rawExercises) : {};
      let parsedDayTypes: Record<string, DayType> = rawDayTypes ? JSON.parse(rawDayTypes) : {};

      // Soft-import legacy items as exercises (only for logged-in users who haven't migrated)
      if (user && rawLegacy && legacyMigrated !== 'true') {
        try {
      ...
      // If user is logged in, fetch from Firebase (Firebase is source of truth)
      if (user) {
        try {
          const progressDocRef = doc(firestore, 'users', user.uid, 'userData', 'progress');
          const progressSnap = await getDoc(progressDocRef);
          if (progressSnap.exists()) {
            const data = progressSnap.data();
            // Use Firebase data as source of truth
            parsedExercises = data.exercisesByDate || {};
            parsedDayTypes = data.dayTypeByDate || {};
            // Update local cache with Firebase data
            try { await AsyncStorage.setItem(STORAGE_EXERCISES_BY_DATE, JSON.stringify(parsedExercises)); } catch {}
            try { await AsyncStorage.setItem(STORAGE_DAYTYPE_BY_DATE, JSON.stringify(parsedDayTypes)); } catch {}
          } else {
            // No Firebase data yet - start fresh for this user
            parsedExercises = {};
            parsedDayTypes = {};
            try { await AsyncStorage.setItem(STORAGE_EXERCISES_BY_DATE, JSON.stringify(parsedExercises)); } catch {}
            try { await AsyncStorage.setItem(STORAGE_DAYTYPE_BY_DATE, JSON.stringify(parsedDayTypes)); } catch {}
          ...
          console.log('[Progress] Firebase fetch failed, using local data:', e);
        }
      } else {
  ...
  const currentRank = React.useMemo(() => getRankForWorkoutDays(totalWorkouts), [totalWorkouts]);
  const nextRank = React.useMemo(() => getNextRankTier(totalWorkouts), [totalWorkouts]);
  const prevTotalRef = React.useRef<number>(0);
  const prevRankRef = React.useRef<RankTier>(getRankForWorkoutDays(0));

  React.useEffect(() => {
    if (!isHydrated) return;
    const prevTotal = prevTotalRef.current;
    const prevRank = prevRankRef.current;
    if (totalWorkouts > prevTotal && currentRank.minDays > prevRank.minDays) {
      setRankJustReached(currentRank);
      setRankModalVisible(true);
    }
    prevTotalRef.current = totalWorkouts;
    prevRankRef.current = currentRank;
  }, [totalWorkouts, currentRank, isHydrated]);
  ...
  async function persistExercises(next: Record<string, Exercise[]>) {
    ...
    if (user) {
      try {
        const progressDocRef = doc(firestore, 'users', user.uid, 'userData', 'progress');
        const sanitized = sanitizeExercisesByDateForFirestore(next);
        await setDoc(progressDocRef, { exercisesByDate: sanitized }, { merge: true });
      } catch (e) {
        console.log('[Progress] Firebase save failed:', e);
      }
    }
  }
  ...
  async function persistDayTypes(next: Record<string, DayType>) {
    ...
    if (user) {
      try {
        const progressDocRef = doc(firestore, 'users', user.uid, 'userData', 'progress');
        await setDoc(progressDocRef, { dayTypeByDate: next }, { merge: true });
      } catch (e) {
        console.log('[Progress] Firebase save failed:', e);
      }
    }
  ...
  return (
    <SafeAreaView style={styles.safeArea}>
      ...
      <View style={styles.rankCard}>
        <View style={[styles.rankIconWrap, { backgroundColor: `${currentRank.color}22` }]}>
          <Ionicons name={currentRank.icon as any} size={22} color={currentRank.color} />
        </View>
        <View style={{ flex: 1 }}>
          ...
          <Text style={styles.rankHint}>
            {nextRank
              ? `${nextRank.minDays - totalWorkouts} more workout ${nextRank.minDays - totalWorkouts === 1 ? 'day' : 'days'} to ${nextRank.name}`
              : 'Top rank unlocked. Keep going!'}
          </Text>
        </View>
      </View>
      ...
      <Modal visible={rankModalVisible} animationType="fade" onRequestClose={() => setRankModalVisible(false)} transparent>
        <View style={styles.rankModalBackdrop}>
          <View style={styles.rankModalCard}>
            <View style={[styles.rankModalIconWrap, { backgroundColor: `${rankJustReached?.color || '#111827'}22` }]}>
              <Ionicons name={(rankJustReached?.icon || 'medal') as any} size={34} color={rankJustReached?.color || '#111827'} />
            </View>
            <Text style={styles.rankModalTitle}>Rank Up!</Text>
            <Text style={styles.rankModalSubtitle}>
              You reached <Text style={[styles.rankModalEmphasis, { color: rankJustReached?.color || '#111827' }]}>{rankJustReached?.name || 'New'}</Text>.
            </Text>
            <TouchableOpacity onPress={() => setRankModalVisible(false)} style={[styles.button, styles.primaryButton, { marginTop: 12, marginLeft: 0 }]}>
...
function getRankForWorkoutDays(totalDays: number): RankTier {
  let current: RankTier = { name: 'Starter', minDays: 0, color: '#6B7280', icon: 'flag' };
  for (const tier of RANK_TIERS) {
    if (totalDays >= tier.minDays) current = tier;
  }
  return current;
}

function getNextRankTier(totalDays: number): RankTier | null {
  for (const tier of RANK_TIERS) {
...
function sanitizeExercisesByDateForFirestore(input: Record<string, Exercise[]>): Record<string, any[]> {
  const out: Record<string, any[]> = {};
  for (const date of Object.keys(input)) {
    const src = Array.isArray(input[date]) ? input[date] : [];
    out[date] = src.map((ex) => {
      const cleaned: Record<string, any> = {
        id: ex.id,
        name: ex.name,
      };
      if (typeof ex.sets === 'number' && Number.isFinite(ex.sets)) cleaned.sets = ex.sets;
      if (typeof ex.repsPerSet === 'number' && Number.isFinite(ex.repsPerSet)) cleaned.repsPerSet = ex.repsPerSet;
      if (typeof ex.minutes === 'number' && Number.isFinite(ex.minutes)) cleaned.minutes = ex.minutes;
      if (typeof ex.notes === 'string' && ex.notes.trim()) cleaned.notes = ex.notes.trim();
      return cleaned;
    });
  }
  return out;
...
const styles = StyleSheet.create({
  ...
  rankLabel: { fontSize: 11, color: '#6B7280', fontWeight: '700', textTransform: 'uppercase', letterSpacing: 0.5 },
  rankName: { fontSize: 18, fontWeight: '800', color: '#111827', marginTop: 2 },
  rankHint: { fontSize: 12, color: '#6B7280', marginTop: 2 },
  calendarCard: {
    marginTop: 12,
    marginHorizontal: 16,
    backgroundColor: '#FFFFFF',
  ...
  rankModalTitle: { fontSize: 22, fontWeight: '800', color: '#111827' },
  rankModalSubtitle: { marginTop: 6, fontSize: 15, color: '#4B5563', textAlign: 'center' },
  rankModalEmphasis: { fontWeight: '800' },
});
